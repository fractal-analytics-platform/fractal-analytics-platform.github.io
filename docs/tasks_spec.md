...
There are five types of tasks in Fractal V2:
...
Tasks have some required parameters, which depend on the task type - see description below. Each task can alos take an arbitrary list of parameters that are specific to the task function and that the user can set.
