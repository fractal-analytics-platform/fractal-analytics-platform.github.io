...
> WARNING: This page is still in-progress, its contents may change based on the
> latest Fractal developments.


## The Fractal image list

Fractal keeps track of all the images it is processing in its image list. The image list has a single entry per OME-Zarr image. Each entry is defined by a unique zarr_url (the full path to the zarr_image). An entry can have optional attributes and types.
Image types are boolean properties that allow to separate the image list into different parts (e.g. 2D vs. 3D with the `is_3D` type, illumination_corrected True vs False if illumination correction was not run in-place). Types can be set both by the task manifest (e.g. after an MIP task, the resulting images always have the type `is_3D`=False) as well as from within an individual task.
Image attributes are scalar properties like booleans, strings, integers or floats. They are always defined from within individual tasks, and never by the task manifest. They allow selecting subsets of your data (e.g. select a given well, a given plate or a given multiplexing acquisition).

Fractal server uses the image list combined with filters (see below) to provide the right zarr_urls to tasks.

Examples on entries:
If one has an OME-Zarr HCS plate with 3 wells that contain an image each, the image list has 3 entries for those 3 images. Each image has attributes for plate & well, as well as the a type (is_3D True or False).
If one has an OME-Zarr HCS plate with 3 wells and each well has 3 multiplexing acquisition, there are 9 OME-Zarr images present. Therefore, the image list has 9 entries and those entries should have the acquisition attributes.

TODO: insert fractal-web screenshot here?
...
### Dataset filter
Examples:
My dataset currently has the filter for type "is_3D" equal to False, because I previously ran an MIP task, and now tasks in the workflow will run on 2D-projected images (unless specifically required).
My dataset currently has the filter for attribute "well" equal to "B03", because I manually added that filter to the dataset (I just want to process a single well for the time being).
...
I manually set it, by modifying the dataset `filters` property.
In a given task entry in the manifest, I include the `output_types` attribute. These types, if set, are automatically included in the dataset filters after the task is run. Examples:
...
Within the task output, I can include a `filters` property, for either image attributes and/or types.
...
TBD: Seeing applied filters during workflow submission & editing filters during submission
...
Attributes & types can also be directly set by a task.
